...
def test_empty_point_gpkg():
    ...
    spt = from_wkb(pt.wkb)
    assert isinstance(spt, ShapelyPoint)
    assert spt.is_empty
...
def test_empty_point(cls, wkb):
    ...
    Test Empty Point
    """
    pt = cls.empty(srs_id=WGS84)
    assert isinstance(pt, cls)
    assert pt.wkb == wkb
...
def test_empty_multi_point(cls, wkb, data):
    ...
    """
    pts = cls([], srs_id=WGS84)
    assert isinstance(pts, cls)
    assert pts.wkb == wkb
    assert not len(pts.coordinates)

    spts = from_wkb(pts.wkb)
    assert isinstance(spts, ShapelyMultiPoint)
    assert spts.is_empty
...
def test_point(header, pt, values, wkb_func, gpkg_func):
    ...
    Test Point
    """
    assert pt.wkb == wkb_func(*values)
    assert pt.as_tuple() == values

    spt = from_wkb(pt.wkb)
    assert isinstance(spt, ShapelyPoint)
    coords, = get_coordinates(spt, include_z=True, include_m=True)
...
def test_multi_point(header, cls, values, wkb_func, env_code, env):
    ...
    assert pts.wkb == wkb_func(values)

    spts = from_wkb(pts.wkb)
    assert isinstance(spts, ShapelyMultiPoint)
    first, second = get_coordinates(spts, include_z=True, include_m=True)
